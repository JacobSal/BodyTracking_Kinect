...
// put this into processbody function and write the joints to a text file!
int CBodyBasics::WriteBody(INT64 nTime, int nBodyCount,const TrackingState& pTrackState, const JointType& pTypeJoint, const CameraSpacePoint& bodyPoint) {
    fstream fout;
    string subjID;
    fout.open("./data_store/subjectdata.csv", ios::out | ios::app);
    fout << nTime;
    fout << ",";
    fout << pTrackState;
    fout << ",";
                    ...
                    if (SUCCEEDED(hr) && bTracked)
                    {
                        ...
                        if (SUCCEEDED(hr))
                        {
                            for (int j = 0; j < _countof(joints); ++j)
                            {
                                ...
                                WriteBody(nTime, nBodyCount, joints[j].TrackingState, joints[j].JointType, joints[j].Position);
                            }
